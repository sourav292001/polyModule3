...
const BASE_PATH = "./circuits/CircuitKashyap/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/CircuitKashyap.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CircuitKashyapVerifier.sol:Verifier");
